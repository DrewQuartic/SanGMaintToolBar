...
Namespace ArcDataBinding
    ...
    Public Class TableWrapper
        ...
        'Private idValue As Integer = 0
        'Private BkPgValue As String = ""
        'Private RmrkValue As String = ""
        ...
        '    End Get
        'End Property
        'Public Property BOOK_PAGE() As String
        '    Get
        '        Return Me.BkPgValue
        '    End Get
        '    Set(ByVal value As String)
        '        If Not (value = BkPgValue) Then
        '            Me.BkPgValue = value
        '            NotifyPropertyChanged("BOOK_PAGE")
        '        End If
        '    End Set
        'End Property
...
#Region "Protected Overrides"
            ...
            If (AllowNew) Then
                ...
                For fieldCount = 0 To NewRow.Fields.FieldCount - 1 Step fieldCount + 1
                    ...
                    If (curField.Editable) Then
                        If m_WrapperCaller = "FrmEditCutLog" Then
                            'Specific for Cut Log Detail table
                            If curField.Name = PARCUTLG_CUT_YEAR_FLD_NAME Then
                                NewRow.Value(fieldCount) = m_CTYR
                            ElseIf curField.Name = PARCUTLG_CUT_NUM_FLD_NAME Then
                                NewRow.Value(fieldCount) = m_CTNM
                            ElseIf curField.Name = PARCUTLG_CUT_YEAR_NUM_FLD_NAME Then
                                NewRow.Value(fieldCount) = m_CTYRNM
                            Else
        ...
        Protected Overrides Sub OnListChanged(ByVal e As ListChangedEventArgs)
            'MsgBox(e.ListChangedType.ToString)
            'MsgBox(Items.IndexOf(e))
            'wrappedTable.Update
            'If e.ListChangedType = ListChangedType.ItemChanged Then
            '    MsgBox(e.OldIndex & " and " & e.NewIndex)
            '    Dim prow As IRow
            '    prow = Item(e.NewIndex)
            '    prow.Store()
            'End If
        End Sub
...
#Region "Private Methods"
        ...
        Private Sub AddData()
            ...
            Dim pbindquery As IQueryFilter
            Dim pQFDef As IQueryFilterDefinition
            pbindquery = New QueryFilter
            pQFDef = pbindquery
            pQFDef.PostfixClause = m_TWPostfixClause
            pbindquery.WhereClause = m_TWWhereClause

            Dim cur As ICursor = wrappedTable.Search(pbindquery, False)
